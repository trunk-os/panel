...
export function usePackages() {
  ...
  const searchPackages = useCallback(async (_params?: PackageSearchParams) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const installPackage = useCallback(async (packageName: string, version = "latest") => {
    try {
      await api.packages.install({ name: packageName, version });
      showToast({ message: `Package ${packageName} installed successfully`, severity: "success" });
      return null; // Real API doesn't return service data immediately
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : `Failed to install package ${packageName}`;
      showToast({ message: errorMessage, severity: "error" });
      throw err;
    }
  }, [showToast]);
