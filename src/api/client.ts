...
export const api = {
  ...
  services: {
    list: async (options?: RequestInit): Promise<ApiResponse<Service[]>> => {
      ...
      console.log(
        `[DEBUG] Found ${containerUnits.length} container units:`,
        containerUnits.map((u) => u.name)
      );
      ...
      console.log(
        `[DEBUG] Container services in final array: ${containerServices.length}`,
    ...
    logs: async (
      ...
      const logsResponse = await api.post<LogMessage[]>(
        "/systemd/log",
        {
          name: serviceId,
          count,
