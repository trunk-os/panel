...
// Mock API client
const createMockApiClient = (shouldSucceed: boolean, statusCode?: number): Pick<ApiClient, 'session'> => ({
  session: {
    me: mock(() => {
      if (shouldSucceed) {
...
describe("authValidation", () => {
  describe("validateStoredToken", () => {
    ...
    test("should return valid when token validation succeeds", async () => {
      const mockApi = createMockApiClient(true);
      const result = await validateStoredToken("valid-token", mockApi as Pick<ApiClient, 'session'>);
    ...
    test("should return expired when 401 error occurs", async () => {
      ...
      const result = await validateStoredToken("expired-token", mockApi as Pick<ApiClient, 'session'>);
    ...
    test("should return invalid with error when 403 error occurs", async () => {
      ...
      const result = await validateStoredToken("forbidden-token", mockApi as Pick<ApiClient, 'session'>);
